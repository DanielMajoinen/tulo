...
export interface GeneratedHooks<Presence, Profile> {
  ...
  useInput(id: string, config?: { skip?: boolean }): Input | null
  useInputUnsuspended(id: string, config?: { skip?: boolean }): { data: Input | null; status: QueryStatus }
  useOneInput: <Config extends HookConfig<InputFilter>>(config?: Config) => Input | null
  useOneInputsUnsuspended: <Config extends HookConfig<InputFilter>>(config?: Config) => { data: Input | null; status: QueryStatus }
  useAllInputs: <Config extends HookConfig<InputFilter>>(config?: Config) => Input[]
  useAllInputsUnsuspended: <Config extends HookConfig<InputFilter>>(config?: Config) => { data: Input[]; status: QueryStatus }
  useAllInputsPaginated: <
    Config extends HookConfig<InputFilter> & {
      pageSize?: number
      suspend?: false
    }
  >(
  ...
  >(
    config?: Config
  ) => [Input[], { loadMore: () => void; hasMore: boolean; status: QueryStatus }]
}
